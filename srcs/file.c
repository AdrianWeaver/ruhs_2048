...
/*   Updated: 2022/03/20 17:27:48 by aweaver          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	new_score(int best, char *pseudo)
{
	...
	unlink("/tmp/temp.txt");
	fd[0] = open("/tmp/temp.txt", O_WRONLY | O_CREAT, 0600);
	if (fd[0] == 0)
		return (1);
	fd[1] = open("score.txt", O_RDONLY | O_CREAT, 0600);
	if (fd[1] == 0)
		return (1);
	...
	if (end == 0)
		return (0);
	fd[0] = open("/tmp/temp.txt", O_RDONLY | O_CREAT, 0600);
	if (fd[0] == 0)
		return (1);
	fd[1] = open("score.txt", O_WRONLY | O_CREAT, 0600);
	if (fd[1] == 0)
		return (1);
	i = 0;
